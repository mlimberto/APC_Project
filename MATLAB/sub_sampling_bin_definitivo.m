...
urm_bin_sampling = urm_bin(index_users,index_items);

icm_sampling = icm_unique(:,index_items);

disp 'Size of URM'
size(urm_bin_sampling)
disp 'Size of ICM'
size(icm_sampling)
...
[r_urm_s,c_urm_s,v_urm_s] = find(urm_bin_sampling);
out_s=[(1:size(urm_bin_sampling,1))',histc(r_urm_s,(1:size(urm_bin_sampling,1))')];

if min(out_s(:,2))<M
    disp('WARNING : for some users m is grater than the number of relevant evaluations');
end
%%
urm_bin_tr=urm_bin_sampling;
for i=1:size(urm_bin_sampling,1)
    clear i_nnz j_nnz v_nnz
    [i_nnz,j_nnz,v_nnz]=find(urm_bin_sampling(i,:));
    perm_index_nnz=randperm(length(j_nnz));
    
    urm_bin_tr(i,j_nnz(perm_index_nnz(1:M)))=0;
end

urm_bin_test=urm_bin_sampling-urm_bin_tr;
...
[ru,cu,vu] = find(urm_bin_sampling);
fileID_urm_sampling=fopen('urm_bin_sampling.txt','wt');
fprintf(fileID_urm_sampling,'%d %d %f\n',[ru-1,cu-1,vu]');
fclose(fileID_urm_sampling);

%% Esportiamo URM_bin_tr

[ru,cu,vu] = find(urm_bin_tr);
fileID_urm_tr=fopen('urm_bin_tr.txt','wt');
fprintf(fileID_urm_tr,'%d %d %f\n',[ru-1,cu-1,vu]');
fclose(fileID_urm_tr);

%% Esportiamo URM_bin_test

[ru,cu,vu] = find(urm_bin_test);
fileID_urm_test=fopen('urm_bin_test.txt','wt');
...
urm_bin_tuning_tr = urm_bin_sampling;
...
urm_bin_tuning_val=urm_bin_sampling-urm_bin_tuning_tr;


%% Esportiamo URM_bin_tuning_tr
